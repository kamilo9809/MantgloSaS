...
import { Wrench, Building2, Globe, Users, PhoneCall, Zap, ShieldCheck, Rocket } from "lucide-react";
  ...
  return (
    <div className="bg-[#f5f5f5] text-[#212121] font-sans">
      ...
      >
        ...
        >
          ...
          <p 
            ...
            >
            Mantenimiento, reparación y soporte técnico confiable para el sector automotriz e industrial.
          </p>
          <a 
      ...
      <section className="max-w-6xl mx-auto py-16 sm:py-20 px-4 sm:px-6">
        <h2 className="text-2xl sm:text-3xl font-bold text-center text-[#0D1B2A]" style={{ fontFamily: 'var(--font-title)' }}>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-6 sm:gap-10 mt-10 sm:mt-14 text-center">
          {[
            ...
            >
              <img src={card.img} alt={card.title} className="w-full h-36 sm:h-44 object-contain rounded-lg mb-4" />
              <h3 className="font-bold text-lg sm:text-xl text-[#0D1B2A] flex items-center justify-center gap-2" style={{ fontFamily: 'var(--font-title)' }}>
                {card.icon} {card.title}
              </h3>
              <p style={{ fontFamily: 'var(--font-body)' }} className="mt-2 text-gray-600 text-sm sm:text-base">{card.desc}</p>
            </motion.div>
          ))}
        </div>
      </section>
      ...
      <section className="bg-[#0D1B2A] text-white py-16 sm:py-20 px-4">
        <div className="max-w-6xl mx-auto grid grid-cols-2 sm:grid-cols-2 md:grid-cols-4 gap-8 sm:gap-10 text-center">
          {[
            { icon: <Wrench className="w-8 h-8 sm:w-10 sm:h-10 mx-auto text-[#FFC107]" />, number: servicios, label: "Servicios realizados" },
            { icon: <Users className="w-8 h-8 sm:w-10 sm:h-10 mx-auto text-[#FFC107]" />, number: experiencia, label: "Años de experiencia" },
            { icon: <Building2 className="w-8 h-8 sm:w-10 sm:h-10 mx-auto text-[#FFC107]" />, number: empresas, label: "Empresas aliadas" },
            { icon: <Globe className="w-8 h-8 sm:w-10 sm:h-10 mx-auto text-[#FFC107]" />, number: cobertura, label: "Ciudades con cobertura" },
          ].map((item, i) => (
            <motion.div
              key={i}
            ...
            >
              {item.icon}
              <p className="text-2xl sm:text-3xl md:text-4xl font-extrabold mt-3" style={{ fontFamily: 'var(--font-title)' }}>{item.number}+</p>
              <p className="mt-1 text-gray-300 text-sm sm:text-base" style={{ fontFamily: 'var(--font-title)' }}>{item.label}</p>
            </motion.div>
          ))}
        </div>
      </section>
      ...
      <section className="py-16 sm:py-20 px-4 sm:px-6 max-w-7xl mx-auto">
        <h3 className="text-2xl sm:text-3xl font-bold text-center text-[#0D1B2A] mb-10 sm:mb-14" style={{ fontFamily: 'var(--font-title)' }}>
          Nuestro taller en acción
        </h3>
        <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-4 sm:gap-6">
          {[images.image5, images.image8, images.image9, images.image10, images.image11, images.image1, images.image2, images.image4].map((img, i) => (
            <motion.img
              key={i}
              src={img}
              alt={`taller${i}`}
      ...
      >
        <div className="max-w-4xl sm:max-w-5xl mx-auto text-center relative z-10">
          <motion.h3
            ...
            style={{ fontFamily: 'var(--font-title)' }}
          >
            La solución que tu empresa <span className="text-amber-400">necesita hoy</span>
          </motion.h3>
          <motion.p
            initial={{ opacity: 0 }}
            ...
            style={{ fontFamily: 'var(--font-body)' }}
          >
            Más de <span className="font-bold text-white">+500 proyectos exitosos</span> y clientes satisfechos nos respaldan.  
            Estamos listos para atenderte <span className="text-amber-400 font-semibold">24/7</span>.
          </motion.p>
        ...
        <div
          ...
          style={{ backgroundImage: "url('https://images.pexels.com/photos/1078884/pexels-photo-1078884.jpeg')" }}
        ></div>
